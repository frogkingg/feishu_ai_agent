...
import { decideMessageGate } from "./agent/message-gate";
import { routeMessageWithRouter } from "./agent/router";
import {
  ...
  matchesCancelText,
  matchesConfirmText,
} from "./memory/confirmation-store";
import { getActiveProjectForChat, summarizeProjectForPrompt } from "./memory/project-store";
import { ProjectPatchDecision as ProjectPatchDecisionV2 } from "./llm/schemas";
import { handleDecisionWorkflow } from "./workflows/decision.workflow";
import {
  commitProjectPatchFromConfirmation,
  handleProjectIntakeWorkflow,
} from "./workflows/project-intake.workflow";
...
function detectSafetyLabel(event: NormalizedMessageEvent): SafetyLabel {
  ...
  if (/(测试一下|试一下机器人|看看你会不会|如果我这么说你会不会|假设我是|假如机器人|研究一下模型|换模型|prompt|提示词)/i.test(text)) {
    return "hypothetical";
  }

  if (/(滚|傻逼|笨比|不用来上班|你可以直接走了)/.test(text)) {
    return "insult";
  }

  if (/(哈哈|笑死|开玩笑|梗|抽卡|泪目)/.test(text)) {
    return "joke";
  }
...
function hasActivityKeyword(text: string) {
  return /(烧烤|聚餐|约饭|吃饭|火锅|团建|出去玩|唱歌|咖啡|看电影|喝酒|夜宵|午饭|晚饭|活动|小聚|寿司|日料|烤肉|牛排|牛扒|餐厅|饭店|自助|小龙虾|披萨|汉堡|拉面|居酒屋|出发|吃(?!吗|不|没|了|吧|嘛|么)[^，,。；;\n？?]{1,20})/.test(
    text,
  );
}

function hasMeetingKeyword(text: string) {
  return /(会议|开会|例会|周会|总结会|复盘会|评审会|同步会|对齐会|OKR|目标制定|目标制订|绩效复盘)/i.test(
    text,
  );
}

function hasGroupPlanningCue(text: string) {
  return /(一起|我们|咱们|他们|她们|大家|全员|全部人|所有人|群内|群里|小伙伴|同学|团队|统一|参加|参会|定在|定到|要不要|去不去|想去|约|安排|改天|拉上|带上)/.test(
    text,
  );
}
...
function hasSameActivityKind(title: string, text: string) {
  if (/(聚餐|吃饭|烧烤|火锅|寿司|日料|烤肉|饭)/i.test(title)) {
    return /(聚餐|吃饭|烧烤|火锅|寿司|日料|烤肉|饭|参加|参与人)/i.test(text);
  }
  if (/(会议|开会|例会|周会|同步会|复盘会|评审会|OKR|目标制定|绩效复盘)/i.test(title)) {
    return /(会议|开会|例会|周会|同步会|复盘会|评审会|OKR|目标制定|绩效复盘|主题|参会|参与人)/i.test(text);
  }
...
function looksLikeNewCalendarTopic(text: string) {
  const hasCreatableOrEvent =
    ...
    (isTimeSupplement(text) && /(会议|开会|例会|周会|同步会|复盘会|评审会|对齐会|聚餐|吃饭|团建|活动|OKR|目标制定|绩效复盘)/i.test(text));
...
function hasCalendarLineEntity(text: string) {
  return (
    ...
    /(流程|出发|拜访|客户|办理|办事|面试|复盘会|目标制定|OKR)/i.test(text)
  );
}
...
function isCalendarLineSeed(text: string) {
  ...
  const hasPeopleOrAction =
    ...
    /(我和|给我和|一起|参加|去|开|定|出发|办理|约|聚|吃|会议|开会|例会|周会|同步会|复盘会|评审会|对齐会)/.test(stripped);
...
function isCalendarLineSupplement(text: string) {
  return (
    ...
    isParticipantAdjustment(text) ||
    /(时间|地点|改到|改成|改为|换成|不如|定在|定到|出发|下班后|参加|参与人|同行)/.test(text)
  );
}
...
function normalizeActivityTitle(title: string | undefined, sourceText: string) {
  ...
  if (cleaned && cleaned.length <= 16 && !cleaned.includes("@")) {
    if (/(烧烤|火锅|烤肉|日料|牛排|牛扒|小龙虾|披萨|汉堡|拉面)/.test(cleaned) && !/(聚餐|吃饭|小聚)/.test(cleaned)) {
...
function isCalendarCreateIntent(text: string) {
  return /(创建|新建|安排|预约|约|添加).{0,12}(日程|日历|会议|同步会|例会|周会|评审会|复盘会|对齐会)/i.test(text);
}
...
async function routeLegacyCalendarMessage(event: NormalizedMessageEvent, context: ChatContext) {
  ...
  console.log(
    `旧日程路由结果: mode=${decision.responseMode} tool=${decision.toolIntent} intent=${decision.intent} confidence=${decision.confidence.toFixed(2)} participants=${decision.participantCandidates.length}`,
  );
...
async function maybeHandleProjectConfirmation(event: NormalizedMessageEvent, activeProject?: ReturnType<typeof getActiveProjectForChat>) {
  ...
  if (!matchesConfirmText(event.text)) {
    return undefined;
  }
  ...
  const patch = confirmed.payload as ProjectPatchDecisionV2 | undefined;
  if (!patch || typeof patch !== "object" || !("action" in patch)) {
    return { type: "text", content: "这条确认没有可写入的项目状态，我先不处理。" } satisfies BotAction;
  }

  try {
    const result = commitProjectPatchFromConfirmation({
      chatId: event.chatId,
      confirmationProjectId: confirmed.projectId,
      patch,
      activeProject,
    });
    ...
    return {
      type: "text",
      content: `项目状态写入失败：${sanitizeError(error)}`,
    } satisfies BotAction;
  }
}
...
async function routeMessage(event: NormalizedMessageEvent) {
  ...
  const activeProject = event.chatId ? getActiveProjectForChat(event.chatId) : undefined;
  const activeProjectSummary = summarizeProjectForPrompt(activeProject);
  const pendingConfirmation =
    event.chatId ? getLatestPendingConfirmation(event.chatId, event.senderId) || getLatestPendingConfirmation(event.chatId) : undefined;
  const gate = decideMessageGate(event, context, pendingConfirmation);
  if (!gate.shouldProcess) {
    return { type: "silent", reason: `message gate: ${gate.reason}` } satisfies BotAction;
  }

  const confirmationAction = await maybeHandleProjectConfirmation(event, activeProject);
  if (confirmationAction) {
    return confirmationAction;
  }

  const route = await routeMessageWithRouter({
    event,
    context,
  ...
  console.log(
    `项目优先路由: domain=${route.primaryDomain} intent=${route.intent} mode=${route.responseMode} safety=${route.safetyLabel} confidence=${route.confidence.toFixed(2)}`,
  );

  if (route.responseMode === "silent" && gate.allowSilent) {
    return { type: "silent", reason: "Router Agent 判断静默" } satisfies BotAction;
  }

  if (route.primaryDomain === "project") {
    const result = await handleProjectIntakeWorkflow({ event, route, context, activeProject, activeProjectSummary });
    return { type: "text", content: result.reply } satisfies BotAction;
  }
  ...
  if (route.primaryDomain === "smalltalk" && (isDirectMention(event) || isPrivateChat(event))) {
    try {
      const llmReply = await callLlm(event, context);
      if (llmReply) {
        return { type: "text", content: llmReply } satisfies BotAction;
      }
    } catch (error) {
      console.error("大模型回复失败:", error);
    }
    return {
      type: "text",
      content: "我在。可以直接和我聊项目推进、任务拆解、风险判断；需要写入项目状态时，我会先请你确认。",
    } satisfies BotAction;
  }

  return { type: "silent", reason: "没有需要处理的项目或日程意图" } satisfies BotAction;
}
