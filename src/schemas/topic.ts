...
export const TopicSuggestedActionSchema = z.enum(["no_action", "observe", "ask_append", "ask_create"]);
...
export const TopicMatchResultSchema = z
  ...
  .superRefine((result, ctx) => {
    ...
    if (result.score < 0.6 && action !== "no_action") {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "score < 0.60 requires no_action", path: ["suggested_action"] });
    }
