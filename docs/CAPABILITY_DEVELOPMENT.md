...
群聊里经常不是“一句话一个命令”，而是先讨论、补时间、补地点、有人同意、最后再说“整理成日程”。因此新增能力要先设计一条 `line`：

- `line.title`：这个话题最终会变成什么，例如“OKR 复盘与目标制定”“汇丰银行取款行程”。
- `line.evidence`：哪些原文支撑它，不允许从别的话题偷信息。
- `line.state`：还在观察、已候选、等待确认、已写入、正在更新，还是关闭。
- `line.next_action`：继续观察、追问、发确认卡片、调用工具、更新已创建对象。

Router 模型只负责“把消息归到哪条线，并压缩这条线”；Tool 模型只负责“这条线现在能不能安全调用工具”。不要让 Tool 模型直接看整段混杂群聊，否则它容易把 KFC 聚餐、OKR 会议和玩笑话串在一起。
