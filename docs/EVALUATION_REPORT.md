...
本文档记录当前 mock fixture 流程验证结果，用于比赛交付物中的效果验证章节。完整机器生成报告见 `evaluation-output/evaluation-report.md`。

重要边界：默认 `mock` 评测使用人工维护的 fixture extraction，不代表真实 LLM 在未知会议上的准确率。它证明的是人工标签、fixture extraction、workflow、topic clustering、confirmation 生成和指标计算可以按预期跑通。真实 LLM 抽取效果需要使用 `EVALUATION_LLM_PROVIDER=openai-compatible` 另行复测。
...
解释：上表中的 100.0% 是 mock fixture pipeline 对固定人工标签的通过率，不是“模型准确率”。它可以用于证明评测框架和 P0 workflow 可复现，但不能直接作为真实 LLM 泛化表现。
...
| Agent dry-run runtime       | 0.02 seconds         |
| Estimated saved time        | 80.0 - 140.0 minutes |
| Estimated efficiency lift   | 100.0%               |

解释：mock 评测使用 fixture extraction，因此 Agent runtime 极短；真实 LLM 复测时该值会随模型延迟上升，但仍可用同一脚本实测。
...
JSON 文件用于机器读取，已被 `.gitignore` 忽略；Markdown 报告可提交。
