...
本文档记录当前 mock 评测结果，用于比赛交付物中的效果验证章节。完整机器生成报告见 `evaluation-output/evaluation-report.md`。
...
| Action item precision           | 100.0%          |
| Owner accuracy                  | 100.0%          |
| Due date accuracy               | 100.0%          |
| Calendar recall / precision     | 100.0% / 100.0% |
| Deadline-vs-calendar accuracy   | 100.0%          |
| Knowledge base trigger accuracy | 100.0%          |
| Decision recall                 | 100.0%          |
| Risk recall                     | 100.0%          |
| Topic keyword recall            | 100.0%          |
...
- 真实模型可能降低或提高指标，报告会保留同一套指标定义，便于比较。
- 自动化测试仍固定为 mock，不依赖真实 LLM。

## 局限性

- 当前样本只有 8 条，适合证明 P0 场景覆盖，不代表大规模生产效果。
- 当前 mock 结果依赖 fixture extraction，主要验证流程、路由和指标计算；真实抽取能力需要用 openai-compatible 复测。
