...
本 Demo 推荐使用模式 A。真实卡片发送测试可以使用模式 B。不要为了发卡片切到模式 C。
...
| 项目       | 模式 A：全 dry-run                                 | 模式 B：只真实发送确认卡片                                           | 模式 C：完整 real mode       |
| ---------- | -------------------------------------------------- | -------------------------------------------------------------------- | ---------------------------- |
| 飞书写入   | 不写真实任务/日程/Wiki/Doc，只记录 dry-run CLI run | 任务、日程、Wiki/Doc 仍然 dry-run                                    | 未来才写任务、日程、知识库   |
| 卡片确认层 | 生成 dry-run card JSON；send-card 只记录 planned   | `FEISHU_CARD_SEND_DRY_RUN=false` 后通过 `larkIm.sendCard` 真实发卡片 | 需单独验收，不推荐直接使用   |
| URL        | 知识库 URL 为 `mock://...`                         | 知识库 URL 仍为 `mock://...`                                         | 应返回真实飞书 Wiki/Doc URL  |
| 安全边界   | `FEISHU_DRY_RUN=true`                              | `FEISHU_DRY_RUN=true`                                                | `FEISHU_DRY_RUN=false`       |
| LLM        | 可以使用 `mock` 或 `openai-compatible`             | 推荐 `mock`，避免模型波动影响飞书链路测试                            | 推荐继续保持 LLM 与写入解耦  |
| Demo 报告  | 记录流程结果，不含密钥或 `.env`                    | 记录真实卡片发送结果，不含 chat id、密钥或 `.env`                    | 真实模式报告同样不应包含密钥 |
| 失败处理   | 记录 planned / dry-run 错误                        | CLI 失败不得伪造 `message_id`                                        | CLI 失败不得伪造业务创建成功 |

当前 P0 Demo 的目标是证明确认链路、状态一致性、主题聚类和知识库建议闭环已经跑通。

真实飞书任务、日程、Wiki/Doc 创建不在本脚本范围内。真实卡片发送只有在模式 B，
也就是显式关闭 `FEISHU_CARD_SEND_DRY_RUN` 且 CLI 已校准时才会发生。
