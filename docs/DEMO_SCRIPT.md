...
在 `FEISHU_CARD_DRY_RUN=true` 下，send-card 不执行真实 CLI，只写 `cli_runs`：
...
真实发送模式需要显式设置 `FEISHU_CARD_DRY_RUN=false`，并保证 `LARK_CLI_BIN`
...
| 卡片确认层 | 生成 dry-run card JSON；可选择 dry-run send-card 记录 | `FEISHU_CARD_DRY_RUN=false` 后通过 `larkIm.sendCard` 真实发卡片 |
...
`FEISHU_CARD_DRY_RUN` 且 CLI 已校准时才会发生。
