...
脚本只验证 P0 Demo 主链路，不会接真实飞书写入，也不会真实发送飞书卡片。
...
当前卡片只是 dry-run card JSON，服务通过 `GET /dev/cards` 和
...
| 卡片确认层 | 生成 dry-run card JSON，可通过 `/dev/cards` 查看 | 通过 `larkIm/sendCard` 发送真实飞书卡片 |
...
真实飞书写入、真实卡片发送和真实 Wiki/Doc 创建不在本脚本范围内。
