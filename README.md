...
最新 RC 验收快照（commit `6b9fb08`）：

- 真实 LLM dry-run canary 已通过：`--dry-check` 和正式 canary 均保持所有 Feishu dry-run 开关为 `true`，`ok=true`，quality 检查通过，未真实写飞书。
- 真实 Feishu task / calendar / Wiki / Doc canary 已通过；该结果仅证明隔离 canary 写入链路，不改变提交版默认 dry-run 安全模式。
- 服务器公网 `/health` 已通过；文档不记录公网域名、密钥或私有主机信息。
- 默认配置仍保持 `FEISHU_DRY_RUN=true` 和 `FEISHU_CARD_SEND_DRY_RUN=true`。
- 飞书妙记事件回调公网验收已通过：`vc.meeting.recording_ready_v1` 合成事件返回 `202 accepted`，重复 `event_id` 返回 `duplicate`，后台事件状态进入 `processed`。卡片按钮公网链路仍按独立验收口径记录。
